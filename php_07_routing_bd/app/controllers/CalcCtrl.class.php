...
class CalcCtrl {
	...
	public function action_calcCompute(){
		...
		if ($this->validate()) {
			...
			if ($db) {
				try {
					$db->insert('calc_results', [
						'kwota' => $this->form->kwota,
						'lata' => $this->form->lata,
						'op' => $this->form->op,
						'rata' => $this->result->result,
						'calkowita' => $this->result->calkowita,
						'odsetki' => $this->result->odsetki,
					]);
					getMessages()->addInfo('Wynik zapisano w bazie danych.');
				} catch (\Throwable $e) {
					getMessages()->addError('Błąd zapisu do bazy: ' . $e->getMessage());
				}
			}
		}
